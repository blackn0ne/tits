...
<template>
    <SeoHead :title="t('admin.categories.title')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-3 px-6 py-3">
            <div class="flex items-start justify-between gap-4">
                <Heading :title="t('admin.categories.title')" :description="t('admin.categories.description')" />
                <Button as-child>
            ...
            <div class="overflow-hidden rounded-xl border bg-card">
                <table class="w-full text-sm">
                    <thead class="border-b bg-muted/50">
                        <tr>
                            <th class="px-4 py-3 text-left font-medium">{{ t('admin.categories.icon') }}</th>
                            <th class="px-4 py-3 text-left font-medium">{{ t('admin.categories.name') }}</th>
                            <th class="px-4 py-3 text-left font-medium">{{ t('admin.categories.slug') }}</th>
                            <th class="px-4 py-3 text-left font-medium">{{ t('admin.categories.status') }}</th>
                    ...
                    <tbody>
                        <tr v-for="category in categories.data" :key="category.id" class="border-b last:border-0">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <Button as-child variant="outline" size="icon" class="size-8">
                                        <Link :href="route('admin.blog-categories.destroy', category.id)" method="delete" as="button">
                                            <Trash2 class="size-4" />
                                        </Link>
                                    </Button>
                                </div>
                        ...
                        <tr v-if="categories.data.length === 0">
                            <td colspan="5" class="px-4 py-8 text-center text-muted-foreground">{{ t('admin.categories.empty') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>

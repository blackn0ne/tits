...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    ...
    {
        title: isEditing.value ? t('admin.blog.edit') : t('admin.blog.create'),
        href: isEditing.value ? route('admin.blog-posts.edit', props.post!.id) : route('admin.blog-posts.create'),
    },
]);
...
const bannerPreview = computed(() => {
    ...
    return props.post?.banner_url ?? null;
});
...
const submit = () => {
    ...
    if (isEditing.value) {
        form.put(route('admin.blog-posts.update', props.post!.id), options);
        return;
    }

    form.post(route('admin.blog-posts.store'), options);
};
</script>

<template>
    <SeoHead :title="isEditing ? t('admin.blog.edit') : t('admin.blog.create')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-3 px-6 py-3 pb-20">
            <Heading :title="isEditing ? t('admin.blog.edit') : t('admin.blog.create')" :description="t('admin.blog.form_description')" />

            <form @submit.prevent="submit" class="w-full max-w-4xl space-y-6">
                <section class="rounded-xl border bg-card p-6 shadow-sm">
                    <h2 class="text-lg font-semibold">{{ t('admin.blog.general') }}</h2>

                    <div class="mt-4 grid gap-4 md:grid-cols-2">
                        <div class="grid gap-2">
                            <Label for="category">{{ t('admin.categories.name') }}</Label>
                            <select
                                id="category"
                        ...
                        <div class="grid gap-2">
                            <Label for="status">{{ t('admin.blog.status') }}</Label>
                            <select
                                id="status"
                                v-model="form.status"
                                class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                ...
                <section class="rounded-xl border bg-card p-6 shadow-sm">
                    <h2 class="text-lg font-semibold">{{ t('admin.blog.banner') }}</h2>
                    ...
                    <div class="mt-4 space-y-4 rounded-lg border border-dashed p-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <Button
                                ...
                                :title="t('admin.blog.remove_banner')"
                                @click="
                ...
                <section class="rounded-xl border bg-card p-6 shadow-sm">
                    <h2 class="text-lg font-semibold">{{ t('admin.blog.content') }}</h2>
                    ...
                    <div v-if="activeTranslation" class="mt-4 grid gap-4">
                        <div class="grid gap-2">
                            <Label>{{ t('admin.blog.post_title') }} ({{ translationsByLanguage[activeLanguageId]?.code }})</Label>
                        ...
                        <div class="grid gap-3 rounded-lg border bg-muted/20 p-4">
                            <p class="text-sm font-semibold">{{ t('admin.blog.seo_section') }}</p>
                            <div class="grid gap-2">
                                <Label>{{ t('admin.blog.meta_title') }}</Label>
                                <Input v-model="activeTranslation.meta_title" />
                                <p class="text-xs text-muted-foreground">{{ t('admin.blog.meta_title_hint') }}</p>
                            </div>
                            <div class="grid gap-2">
                                <Label>{{ t('admin.blog.meta_description') }}</Label>
                                <textarea
                                ...
                                />
                                <p class="text-xs text-muted-foreground">{{ t('admin.blog.meta_description_hint') }}</p>
                            </div>
                        </div>
                        <div class="grid gap-2">
                            <Label>{{ t('admin.blog.body') }}</Label>
                ...
                <div class="flex justify-end gap-2">
                    <Button type="button" variant="outline" @click="router.visit(route('admin.blog-posts.index'))">
                        {{ t('admin.categories.cancel') }}
                    </Button>
                    <Button type="submit" :disabled="form.processing">{{ t('admin.blog.save') }}</Button>
                </div>
            </form>
        </div>
    </AppLayout>
